...
TODO: Re-index the section header numbers
...
- **Cross-Game Transfer.** Because games share a common relational representation, 
...
TODO: This table and surrounding prose needs updating to reflect the new schema which has multiple edge tables.
| Object | Meaning |
|--------|---------|
...
Two foreign keys record which square each piece occupies, and two more record 
the source and target squares of each edge. Let's look at how we implement this schema in RewriteGames.jl,
or more accurately how RewriteGames.jl uses Catlab.jl and ACSets.jl to do so.
...
#| fig-cap: "The naturality condition: preserving relationships across instances."
\begin{tikzpicture}
  \node (A) at (0,2) {$I(c)$};
  \node (B) at (2,2) {$J(c)$};
  \node (C) at (0,0) {$I(d)$};
  \node (D) at (2,0) {$J(d)$};
  \draw[->] (A) -- (B) node[midway,above] {$h_c$};
  \draw[->] (A) -- (C) node[midway,left] {$I(f)$};
  \draw[->] (C) -- (D) node[midway,below] {$h_d$};
  \draw[->] (B) -- (D) node[midway,right] {$J(f)$};
\end{tikzpicture}
```
...
```{tikz}
#| label: fig-span-placeholder
#| fig-cap: "Placeholder: A span $L \\leftarrow K \\to R$ showing example ACSets (e.g., for marking an X)."
\begin{tikzpicture}[box/.style={draw, minimum width=2cm, minimum height=1.5cm, align=center}]
  \node[box] (L) at (0,0) {$L$\\[1em]\tiny (Pattern to find)};
  \node[box] (K) at (4,0) {$K$\\[1em]\tiny (Preserved structure)};
  \node[box] (R) at (8,0) {$R$\\[1em]\tiny (Replacement structure)};
  
  \draw[->] (K) -- (L) node[midway,above] {$l$};
  \draw[->] (K) -- (R) node[midway,above] {$r$};
  
  \node at (0,-1.5) {\textit{Example: Square with X?}};
  \node at (4,-1.5) {\textit{Example: Bare Square}};
  \node at (8,-1.5) {\textit{Example: Square with O?}};
\end{tikzpicture}
```
...
#| fig-cap: "The Double Pushout (DPO) diagram."
\begin{tikzpicture}
  \node (L) at (0,2) {$L$};
  \node (K) at (2,2) {$K$};
  \node (R) at (4,2) {$R$};
  ...
  \draw[->] (K) -- (D) node[midway,left] {$k$};
  \draw[->] (R) -- (Gprime) node[midway,right] {$m'$};
  \draw[->] (D) -- (G) node[midway,below] {$g$};
  \draw[->] (D) -- (Gprime) node[midway,below] {$g'$};
\end{tikzpicture}
```

### What is a Pushout?

Formally, a **pushout** of two morphisms $f: A \to B$ and $g: A \to C$ consists 
of an object $P$ and two morphisms $f': C \to P$ and $g': B \to P$ such that 
$f' \circ g = g' \circ f$. 

Furthermore, it must satisfy the **universal property**: for any other object 
$Q$ and morphisms $h: C \to Q$ and $k: B \to Q$ such that $h \circ g = k \circ f$, 
there exists a unique morphism $u: P \to Q$ such that $u \circ f' = h$ and 
$u \circ g' = k$.

```{tikz}
#| label: fig-pushout-universal
#| fig-cap: "The universal property of a pushout."
\begin{tikzpicture}
  \node (A) at (0,2) {$A$};
  \node (B) at (2,2) {$B$};
  \node (C) at (0,0) {$C$};
  ...
  \draw[->] (A) -- (C) node[midway,left] {$g$};
  \draw[->] (B) -- (P) node[midway,right] {$g'$};
  \draw[->] (C) -- (P) node[midway,below] {$f'$};
  
  \draw[->,bend left] (B) to node[midway,above right] {$k$} (Q);
  \draw[->,bend right] (C) to node[midway,below left] {$h$} (Q);
  \draw[->,dashed] (P) -- (Q) node[midway,above] {$u$};
\end{tikzpicture}
```

In the context of ACSets, this means taking the disjoint union of the 
elements in $B$ and $C$, and then identifying (merging) any elements that 
came from the same element in $A$.
...
**minimal** object that combines $B$ and $C$ along $A$. 

-   **No extra elements:** The property ensures we haven't added any 
    "ghost" squares or pieces that weren't in $B$ or $C$.
...
### Taking Stock of the Rewrite Process

Before we dive into implementation, let's recap the workflow of a structural 
rewrite. The lifecycle of a move in RewriteGames.jl follows these steps:
...
3.  **Determining What to Keep (The Pushout Complement $D$):** Once a match 
    is chosen, the engine computes the **pushout complement**. This step 
    effectively "deletes" the parts of the board matched by $L$ that aren't 
    part of the interface $K$, leaving us with a "residual" board $D$.

```{tikz}
#| label: fig-pushout-complement-placeholder
#| fig-cap: "Placeholder: Visualizing the pushout complement (finding $D$ from $G$, $L$, and $K$)."
\begin{tikzpicture}[box/.style={draw, minimum width=2cm, minimum height=1.5cm, align=center}]
  \node[box] (G) at (0,0) {$G$\\[1em]\tiny (Current World)};
  \node[box] (L) at (0,3) {$L$\\[1em]\tiny (Pattern)};
  \node[box] (K) at (3,3) {$K$\\[1em]\tiny (Interface)};
  \node[box, dashed] (D) at (3,0) {$D$\\[1em]\tiny (Remaining World)};
  
  \draw[->] (L) -- (G) node[midway,left] {$m$};
  \draw[<-] (L) -- (K) node[midway,above] {$l$};
  \draw[->] (K) -- (D) node[midway,right] {$k$};
  \draw[->] (D) -- (G) node[midway,below] {$g$};
...
4.  **The Result (The RHS Pushout $G'$):** Finally, the engine computes a 
    second pushout, gluing the new pattern $R$ onto the residual board $D$ 
    along the shared interface $K$. This produces the updated world $G'$.
...
The `mark_x` rule is the fundamental move for the X player: it places an X 
piece on any empty square. In our DPO framework, this is represented by a 
span that "keeps" a square and "adds" an X piece to it.

#### Understanding the Span Legs

-   **$K \to L$ (The Left Leg):** For `mark_x`, we use the identity 
    morphism on `Sq` ($id_{Sq}$). Because this map is an isomorphism, it 
    tells the engine that **nothing is deleted**. We are simply finding a 
    square and keeping it exactly as it is.
-   **$K \to R$ (The Right Leg):** We use the unique morphism from `Sq` 
    into the representable `X`. This map "embeds" our bare square into a 
    structure that also contains an X piece. Categorically, this represents 
    the **addition** of the piece.

By finding a match for $L$ (a square) and applying the DPO process, we 
effectively glue a new X piece onto that specific square on the board.
...
#### Enforcing Legality with NACs

While the span describes *what* the move does, it doesn't yet say that we 
can't place a piece on a square that's already occupied. For this, we use 
**Negative Application Conditions (NACs)**.

Think of a NAC as a "forbidden extension." We tell the engine: "You can match 
this bare square $L$, **unless** that square is already part of a larger 
pattern $N$."

For `mark_x`, we have two NACs:
1.  **"Not already an X":** Reject the match if the square already has an X.
2.  **"Not already an O":** Reject the match if the square already has an O.

In the code below, `homomorphism(Sq, X; cat=𝒞)` defines the extension from a 
bare square to a square-with-X. If this extension can be found in the 
current board state for our matched square, the NAC "fires" and the match is 
rejected.
...
TODO: I think the following "RuleApp" subsection makes more sense in the "Building the Schedules" section. There, we should start out by "Taking Stock", and then describe RuleApp, and then describe how to arrange into a schedule.
...
TODO: Hand hold the reader a bit more about the consequences of tagging a player. Hint at where players are registered and say it will be explained later on.
...
TODO: This needs much more hand holding explanation. Give intuition about why we thinking of "trace". Perhaps we need to show a simple schedule somewhere before this so people
can think about what "wires" are. Actually, I think it might make sense to put this discussion *after* the X and ) schedules.
...
TODO: Hand hold the user a bit more through the process of constructing a Game and running training.
