...
**double-pushout (DPO)** construction in two pushout steps:

1. **Identify the context**: construct the pushout complement `C` by removing from `G`
   everything matched by `L` that is not in the preserved interface `K`.  Concretely,
   delete all parts of `G` that map to elements of `L ∖ K`.  For `mark_x`, the interface
   `K = L = Sq`, so *nothing* is deleted.
2. **Glue in the replacement**: take the pushout of `K → R` and `K → C` to produce the
   updated world `G'`.  For `mark_x`, this adds one `X` part to `G` with its `xsq`
   foreign key set to the matched square.

A concrete example with `mark_x` applied to a board where square 5 (centre) is empty:

```
L = K = {Sq:1}                     R = {Sq:1, X:1}  (X:1 points to Sq:1)

        m maps Sq:1 ↦ square 5 in G

G  (board, no pieces) ──DPO──▶  G' (board, X on square 5)
```

The rewrite is fully determined by the span `L ← K → R` together with the match morphism
`m`.  The engine enumerates all valid `m`s; the player (via `PlayerRuleApp`) selects one.
...
```
NAC 1 — square already has X:   Sq ──homomorphism──▶ X   (L → N where N = X representable)
NAC 2 — square already has O:   Sq ──homomorphism──▶ O   (L → N where N = O representable)
```

A proposed match `m : Sq → G` (mapping the pattern square to some board square) is
rejected if the X-piece embedding *or* the O-piece embedding can be extended consistently
to the board — i.e., if the matched square already holds any piece.
...
just an ACSet pattern that can be embedded in the current board — if the embedding exists,
X (or O) has won.  No-op rules (where `L = K = R = pattern`) leave the board unchanged
but route execution to a "success" wire when a match is found, allowing the schedule to
branch on the result.

There are two natural ways to encode the eight winning lines.

**Approach 1 — Hardcoded configurations.**  Enumerate all eight winning lines explicitly.
For each line build a complete 9-square board (with the board's full edge topology) and
mark exactly the three winning squares with X pieces.  A homomorphism from this pattern
into the board exists if and only if those three squares all hold X.

The ACSet mutating functions used below:

- `add_parts!(acset, :Type, n)` — adds `n` elements of the given type, returning their IDs.
- `add_part!(acset, :Type; key=val, ...)` — adds one element and sets its foreign keys.

```{julia}
#| label: hardcoded-win-patterns
function x_win_board(squares)
    b = create_board()          # full 9-square board topology
    for sq in squares
        add_part!(b, :X, xsq=sq)
    end
    return b
...
x_hardcoded_rules = [Rule(id[𝒞](x_win_board(l)), id[𝒞](x_win_board(l))) for l in winning_lines]
```

This is easy to audit — each pattern maps directly to one named winning line.  The cost
is eight separate rules that must all be checked, and any board-size change requires
regenerating patterns by hand.

**Approach 2 — Structural patterns.**  Rather than listing each winning line, describe
*what makes* three squares a winning line: they must be connected by two successive edges
in the board graph (for rows and columns), or by a specific edge fork/join (for
diagonals).  A single connected-path pattern then matches *any* three-in-a-row,
regardless of which specific squares they happen to be.

This approach scales automatically to larger boards and generalises to any game where
"three connected pieces in a row" constitutes a win.  We use it for the rest of the
tutorial because it compresses eight patterns into three, and the migration functor
produces the O-side patterns for free.

Walk-through for each structural pattern — using the same `add_parts!` / `add_part!`
helpers from above:

1. **Row or column pattern** (`row_col_structural`): start with 3 squares, add edges
   `1→2` and `2→3` forming a directed path, then mark all three squares X.  Horizontal
   and vertical edges share the same direction convention in the board, so one path
   pattern matches both row and column wins.
2. **Main diagonal pattern** (`diag_structural_1`, squares 1–5–9): the diagonal hops one
   horizontal edge then one vertical edge.  A 5-node path `1→2→3→4→5` with X on the
   odd nodes (1, 3, 5) captures this skip-step structure.
3. **Anti-diagonal pattern** (`diag_structural_2`, squares 3–5–7): the anti-diagonal
   reverses the horizontal direction.  A fork/join with 5 nodes where edges `1→2`,
   `3→2`, `3→4`, `5→4` create two "V" shapes with X on nodes 1, 3, 5 captures it.
```{julia}
#| label: win-patterns
...
- **`PlayerRuleApp`** wraps a `Rule` and tags it with a player identity.  When execution
  reaches this box the engine asks the player's agent to choose a move rather than
...
records an `Experience`.  If no matches exist (board full), the box routes to its failure
port, signalling a tie.

Like a plain `RuleApp`, a `PlayerRuleApp` has two output ports: **success** (a move was
made) and **failure** (no legal moves exist).
...
success paths into one exit wire.
...
port 2 fires if no empty square exists (tie).  After a move the win-check sub-schedule
determines whether X has just won.
...
rule's right-hand side now adds an `O` piece, the win-check rules look for three `O`
pieces in a row, and the `PlayerRuleApp` player tag has been updated from `:X` to `:O`.

### Full game loop

The outer schedule loops X then O, repeating until someone wins or the board fills.  The
`trace_arg` wire loops the continuing board back for the next round; `init` carries the
fresh board on the first iteration.  Both are merged before entering `X_sched_gs`.
...
The first return wire `o_not` is the trace wire (fed back for the next round); the
second `game_over` is the exit wire.
...
- `terminal` is called after every move.  It returns a `(Bool, Symbol|Nothing)` pair:
  `(true, :X)` means X won, `(true, nothing)` means a draw, `(false, nothing)` means
  play continues.
...
pre-computed GNN-ready tensors.  Agents that need only the raw world access
`state.raw.world` directly.
...
After the episode, `exps[end].next_state.raw.world` holds the final board ACSet:
