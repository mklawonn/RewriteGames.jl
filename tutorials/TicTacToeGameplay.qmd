...
function reinforce_loss(model, batch::Vector{StepRecord}, returns::Vector{Float32})
    total = 0f0
    for (rec, G) in zip(batch, returns)
        g, _ = world_to_gnn(rec.world)
...
function train_self_play!(model, opt_state, game, game_sched;
    ...
    for update in 1:n_updates
        ...
        loss, grads = Flux.withgradient(m -> reinforce_loss(m, batch, returns), model)
...
t_grad = @elapsed begin
    loss_val, grads = Flux.withgradient(
        m -> reinforce_loss(m, test_batch, test_returns), gnn_model)
